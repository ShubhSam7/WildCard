...
export function PerformanceMatrix({ loading = true, className }: PerformanceMatrixProps) {
  if (loading) {
    return (
...
function MetricItem({
  ...
  return (
    <div className="flex items-center gap-3">
      <div className={cn("w-10 h-10 rounded-lg bg-surface-low flex items-center justify-center", iconColor)}>
        <Icon className="w-5 h-5" />
      </div>
      <div className="flex-1">
        <p className="text-xs text-on-variant">{label}</p>
        <p className="text-lg font-bold text-on-surface font-manrope">{value}</p>
      </div>
    </div>
  );
}
